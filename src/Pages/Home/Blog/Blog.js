...
const Blog = () => {
  ...
  return (
    ...
    data-aos-duration="2000" className="py-28">
      ...
      <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 my-10 w-10/12 mx-auto">
        {blogData.map((blog) => (
          <div
            key={blog.id}
            blog={blog}
            className="card card-compact  bg-white text-black shadow-xl"
          >
            <figure>
              <img src={blog.image} alt="img" />
            </figure>
            <div className="card-body ">
              <h2 className="card-title mb-5">{blog.title}</h2>
              {/* <p>{blog.title}</p> */}
              <Link to={`/blog/${blog.id}`} state={{ from: { blog } }}>
                <div class="">
                  <button className="btn bg-secondary hover:bg-secondary text-black btn-sm border-none">Fitlessian Blog</button>
                </div>
              </Link>
            </div>
          </div>
