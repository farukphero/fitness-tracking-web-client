...
const FoodInfoSlide = ({ result, startDate, setStartDate }) => {
  ...
  const userBmr = (user) => {
    if (user?.userInfo?.gender === "Male") {
      return (
        88.36 +
        13.4 * parseInt(user?.userInfo?.weight) +
        4.8 * (5.5 * 30.48) -
        5.7 * parseInt(user?.userInfo?.age)
      );
    } else if (user?.userInfo?.gender === "female") {
      return (
        447.6 +
        9.2 * parseInt(user?.userInfo?.weight) +
        3.1 * (5.5 * 30.48) -
        4.3 * parseInt(user?.userInfo?.age)
      );
    }
  };
  ...
  const pal = TEE / bmr;
  const dailyCaloryRequirementMaintanance = pal * bmr;
  const dailyCaloryRequirementGain = parseInt(
    dailyCaloryRequirementMaintanance + extraCalorey
  );
  const dailyCaloryRequirementLose = parseInt(
    dailyCaloryRequirementMaintanance - extraCalorey
  );
          ...
          <SwiperSlide>
            <div className="pb-10 text-center">
              <h3 className="text-2xl">
                Food Plan (how much you should eat for the day)
              </h3>
            </div>
            <div className="flex items-center justify-center space-x-6 capitalize text-center">
              <div className="avatar">
              ...
              <h2 className="font-bold text-xl">
                ...
                <span className="text-green-500">{cal}</span> cals
              </h2>
            </div>
          </SwiperSlide>
          <SwiperSlide>
            <div className="pb-10 text-center">
              <h3 className="text-2xl">
                Calories in vs out (are you on track right now)
              </h3>
            </div>
            <div className=" flex items-center justify-center space-x-6 capitalize text-center">
              <div className="avatar">
