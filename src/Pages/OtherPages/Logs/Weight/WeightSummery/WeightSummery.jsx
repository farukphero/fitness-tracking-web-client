...
const WeightSummery = ({setLogedWeight,refetch}) => {
    const { user, userInfo } = useContext(AuthContext)
    const [goalSetData, setGoalSetData] = useState();
    const [logedData, setLogedData] = useState();
    const date = Math.abs((new Date(goalSetData?.date)) - (new Date(logedData?.date)));
    ...
    return (
        <div className='flex'>
            <div className='lg:mx-4 w-3/4 mx-auto'>
                ...
                {
                    goalSetData && <div className="border border-gray-500 p-4 rounded my-4 lg:w-1/2 mx-auto shadow-xl ">
                        <h1 className="font-bold text-center text-2xl border-b pb-2 border-gray-500">Suggestion For You</h1>
                        {
                            goalSetData.goalType === "Gain" ?
                                <>
                                    <p className='text-center text-gray-300 my-3'>You have to take <span className='text-white font-bold'>{dailyTake.toFixed(2)}</span> calorie daily to reach your goal.</p>
                                </> :
                                <>
                                    <p className='text-center text-gray-300 my-3'>You have to burned <span className='text-white font-bold'>{dailyTake.toFixed(2)}</span> calorie daily to reach your goal.</p>
                                </>
                        }
                        ...
                        <>
                            ...
                            <>
                                <p className='text-black text-center font-bold'>You want to lose {oneDayWeight.toFixed(2)} kg in one day, But it is harmful to your health. We recommend that you will lose 0.07 kg to 0.14 kg in one day. That means 0.5 kg to 1 kg per week.
                                </p>
                            </>
                        }
                        </div>
            ...
            <div className='lg:mx-4 w-1/4 mx-auto'>
                <div className=''>
                    {/* goal set */}
