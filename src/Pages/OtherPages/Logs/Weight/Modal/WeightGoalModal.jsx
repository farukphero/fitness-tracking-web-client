...
const WeightGoalModal = ({ logedInfo, setRestWeight,setWeightGoal }) => {
    ...
    const { data: expectedWeightGoal = [], refetch } = useQuery({
        ...
        queryFn: () => fetch(`http://localhost:5000/weightGoal?email=${user?.email}`)
            .then(res => res.json())
            .then(data => {
    ...
    const weightGoal = data => {
        ...
        fetch(`http://localhost:5000/weightGoal/${user?.email}`, {
            method: "PATCH",
            headers: {
    ...
    return (
        <div>
            ...
            <input type="checkbox" id="set-weight-goal-modal" className="modal-toggle" />
            <div className="modal ">
                <div className="modal-box relative bg-gray-600 ">
                    <label htmlFor="set-weight-goal-modal" className="bg-gray-600 btn btn-sm btn-circle absolute right-2 top-2">✕</label>
                    ...
                    <form onSubmit={handleSubmit(weightGoal)}>
                        <div className="form-control">
                            ...
                            <div className=' flex'>
                                <input className='bg-gray-500 mr-1 text-center rounded' type="number" {...register("expectedWeight", {
                        ...
                        <div className="modal-action">
                            <button>
                                <label htmlFor="set-weight-goal-modal" className="btn btn-sm text-center btn-outline text-white bg-gradient-to-r from-gray-800 to-green-400 ">Submit</label>
                            </button>
                        </div>
                    </form>
